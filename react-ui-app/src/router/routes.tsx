...
const lazyLoad = (path: string) => {
    const fullPath = path?.startsWith('/') ? `../features${path}.tsx` : `../features/${path}.tsx`
    if (modules[fullPath]) {
        return lazy(modules[fullPath])
    } else {
        return lazy(() => Promise.reject(new Error(`Module not found: ${fullPath}`)));
    }
}
...
const flatMap = (routes: RouteType[]): RouteObject[] => {
    return routes.map(route => {
        return {
            path: route.path,
            element: route?.component ? lazyImportComponent({lazyChildren: lazyLoad(route.component)}) :
                <BlankLayout/>,
            meta: route.meta,
